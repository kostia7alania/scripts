...
// = отчетРЕГИ:
iimSet('строка',строка); iimSet("имяПроекта","TJ"); iimSet("действие","REG"); iimSet("папкаОтчетов","C:\\avito.ru\\TJ\\результаты"); iimSet('num'," "); iimSet("телефон"," "); iimPlay("GO//GO-REPORT.iim")
...
iimDisplay(имяПроекта+ ": Го добавлять объяву ужеж в акк №: "+строка);
			...
			//==============TJ1:=============
do{iimSet('строка',строка);iimSet('num',1); iimSet('путьПроекта',"C:\\avito.ru\\TJ"); iimSet('почта',почта); iimSet('пароль',пароль);
iimSet('метроИлиГород','all_metros.txt')//=область-замануха!!;
iimSet('имяПроекта',"TJ");
...
iimSet('фото-для-подачи','C:\\фотки-TJ'); iimSet('x',x); iimPlay("GO//go-reload-picture.iim")
// = ОТЧЕТ:
iimSet('строка',строка); iimSet("имяПроекта","TJ");iimSet("действие","add"); iimSet("папкаОтчетов","C:\\avito.ru\\TJ\\результаты"); iimSet("телефон"," "); iimSet('num',"-1-"); iimPlay("GO//GO-REPORT.iim")


			//==============TJ2:=============
do{iimSet('строка',строка);iimSet('num',2); iimSet('путьПроекта',"C:\\avito.ru\\TJ"); iimSet('имяПроекта',"TJ"); iimSet('почта',почта); iimSet('пароль',пароль);
...
//= обходим ОШИБКУ ЗАГРУЗКИ ФОТО:
iimSet('фото-для-подачи','C:\\фотки_пк'); iimSet('x',x); iimPlay("GO//go-reload-picture.iim")
// = ОТЧЕТ:
iimSet('строка',строка); iimSet("имяПроекта","TJ");iimSet("действие","add"); iimSet("папкаОтчетов","C:\\avito.ru\\TJ\\результаты"); iimSet("телефон"," "); iimSet('num',"-2-"); iimPlay("GO//GO-REPORT.iim")
