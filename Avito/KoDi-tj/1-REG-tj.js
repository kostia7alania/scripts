...
for(var строка=СоСтроки;строка<ДоСтроки+1;строка++)
{
	...
	//отчетКОСТЯ:
	var отчет=`SET !REPLAYSPEED fast\n SET !USERAGENT {{useragent}}\n SET !ERRORIGNORE YES\ntab t=2\nURL GOTO=https://m.avito.ru/profile/settings\nwait seconds=2\nSCREENSHOT TYPE=Page FOLDER=C:\\Users\\user\\OneDrive\\tj\\результаты\\скрины FILE={{строка}}---1РЕГ-tj-{{!NOW:dd.mm.yyyy}}-[{{!NOW:hh:nn}}].jpg\n SET !EXTRACT #{{строка}},<sp>{{!NOW:dd.mm.yyyy}}<sp>[{{!NOW:hh:nn}}],<sp>{{!URLCURRENT}},<sp>{{айпишки}}\n SAVEAS TYPE=EXTRACT FOLDER=C:\\Users\\user\\OneDrive\\tj\\результаты FILE=1-РЕГ-tj.txt\nTab t=1`;
	...
	function смсКопиИподтвердить() {iimPlayCode('SET !REPLAYSPEED fast\nSET !TIMEOUT_STEP 0\nTAB T=0'); sms=копиБоди(); sms=sms.split(':')[2]; sms=sms.split(',')[0]; sms=sms.split('.')[0];sms=sms.split('"')[1]; iimSet("sms",sms); iimSet("useragent", useragent); iimPlayCode(ПодтвердитьСмс);}
    ...
    iimPlayCode(пауза.сек5)
	if(getNum1()=='1') //если на балансе есть бабки-идем дальше;-иначе-алерт;что нет бабок;
	{ 	sms6:
...
//////////GLOBAL//////////GLOBAL//////////GLOBAL//////////GLOBAL//////////GLOBAL//////////GLOBAL//////////GLOBAL//////////GLOBAL//////////GLOBAL//////////			
				...
				{	iimDisplay("все ок, т.е. тел не by и капча тож подошла; с 1 раза--идем дальше:все идеально;");  
					...
					{	iimSet('x',кодОперации); 
						...
						{	iimDisplay("НЕВЕРНЫЙ КОД АКТИВАЦИИ. Сейчас на акке:"+строка+"("+ДоСтроки+")");
							...
							{	iimSet('x',кодОперации); 
								iimPlayCode(getState); 
								iimDisplay("ждем смс-код; Проверка №"+jjjj+"(25); Сейчас регаем акк: "+строка+"("+ДоСтроки+")"); 
				...
				{	iimDisplay("проверка - в чем ИМЕННО загвоздка возникла в НАЧАЛЕ");
					...
					{	if (тестТелефона!=-1){iimDisplay("спалили баяниский телефон!!!"); iimSet('x',кодОперации);iimPlayCode(setOperationUsed); форКапча=11; iimPlayCode(пауза.сек5); break}
						...
						iimDisplay('instead-ниуя не спалили-waiting')
...
//////////GLOBALend//////////GLOBALend//////////GLOBALend//////////GLOBAL//////////GLOBAL//////////GLOBAL//////////GLOBAL//////////GLOBAL//////////GLOBAL//////////GLOBAL				
	...
	{	iimDisplay("бабки положи - и перезапускай!"); alert('бабкии закончлись!;');	}; 
}			
