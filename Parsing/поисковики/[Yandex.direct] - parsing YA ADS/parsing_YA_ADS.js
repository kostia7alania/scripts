...
var обработанные="C:\\yandex\\обработанные.txt";	// база уже обработанных сайтов
var путьСохр="C:\\yandex\\result.txt";		// КУДА СОХРАНЯЕМ;
...
for(q=0;q<ключ.length;q++){
	iimSet('ключ',ключ[q]); 
	iimSet('регион',регион); 	
iim('URL GOTO=https://yandex.ru/search/ads?text={{ключ}}&lr={{регион}}');
try{тестНаКаптчу()}catch(e){}
	...
	for(zz=0;zz<30;zz++){
			...
			for (var i = 0; i < elements1.length; i++) {
				// alert(elements1[i]+"\n\n\n\n"+elements2[i])
					try{var url = elements2[i].innerText}catch(e){}
					...
					try{var результат="\n#"+q+"."+zz+"."+i+"	"+url+"	"+elements1[i].innerText;}catch(e){alert('err')};
			  ...
			  if(iimPlayCode(`set !timeout_tag 2\nTAG POS=1 TYPE=A ATTR=TXT:дальше`)>0){//след.стр существует,продолжаем.
				  }else{
