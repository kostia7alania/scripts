...
iimSet("айпишки",айпишки); iimSet("строка",i);
	...
	URL GOTO=https://m.avito.ru/add
						...
						SET этаж EVAL("var min = 2; var max = 7; var randomNumber = Math.floor(Math.random() * (max - min + 1)) + min; randomNumber;")
						SET этажей EVAL("var min = 8; var max = 25; var randomNumber = Math.floor(Math.random() * (max - min + 1)) + min; randomNumber;")
						...
						TAG POS=1 TYPE=SELECT FORM=NAME:NoFormName ATTR=ID:param_504 CONTENT=%5257
							'площадь
						SET площадь EVAL("var randomNumber=Math.floor(Math.random()*(42-22+1)+22); randomNumber;")
