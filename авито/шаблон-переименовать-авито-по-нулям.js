...
for(asd=0;asd<3;asd++){
	//общие настройки:
	iimPlayCode('SET !REPLAYSPEED fast\nCMDLINE !DATASOURCE C:\\авито-переменные\\началоПочты.txt\nSET !DATASOURCE_LINE 1\nset !extract {{!COL1}}'); var началоПочты=iimGetLastExtract(1);
	iimPlayCode('SET !REPLAYSPEED fast\nCMDLINE !DATASOURCE C:\\авито-переменные\\конецПочты.txt\nSET !DATASOURCE_LINE 1\nset !extract {{!COL1}}'); var конецПочты=iimGetLastExtract(1);
	iimPlayCode('SET !REPLAYSPEED fast\nCMDLINE !DATASOURCE C:\\авито-переменные\\пароль.txt\nSET !DATASOURCE_LINE 1\nset !extract {{!COL1}}');var пароль=iimGetLastExtract(1);
	 //хдесь задается строка!!!!==>\|/:
	iimSet("имяПроекта",имяПроекта); iimPlayCode("SET !DATASOURCE {{имяПроекта}}-текущийАкк-рин.txt\nSET !DATASOURCE_LINE 1\nSET !EXTRACT {{!COL1}}"); var строка = Number(iimGetLastExtract(1)) //задаем № тек.акк ринА;
	iimSet("имяПроекта",имяПроекта); 
	iimPlayCode("SET !DATASOURCE {{имяПроекта}}-текущийАкк.txt\nSET !DATASOURCE_LINE 1\nSET !EXTRACT {{!COL1}}"); var строкаРеги = Number(iimGetLastExtract(1)) //задаем акк реги--для сравнения;
	iimDisplay('разница между строкаРеги-строкиРинейма = '+(строкаРеги-строка)+"; >0? = "+(строкаРеги-строка>0))
	...
	iimSet('путьИмениСТелефоном',путьИмениСТелефоном);
		...
		iimSet("папкаСкринов",папкаСкринов);
			...
			if(строкаРеги-строка>0){iimDisplay('строкаРеги-строкиРинейма='+(строкаРеги-строка)+"; >0? = "+(строкаРеги-строка>0));
			continue GoAgainRename}
			else{break}
	};
	if(имяПроекта=="tj"){var имяПроекта="pc"};
	if(имяПроекта=="kv"){var имяПроекта="tj"};
};