var robot = {
	...
	random_txt(){
 	...
 	url_counter: 0,
	accs() {return read(`${this.path}logins.csv`)},
...
var PostToGroup = (url, post, img) => {
	...
	try {// try share post
		...
		iim(wait(2));
		iim(" set !TIMEOUT_STEP 1 \n TAG POS=1 TYPE=SPAN ATTR=TXT:Администратор<SP>запретил<SP>делиться<SP>темами<SP>в<SP>этой*"); //без этой фигни след команда не срабатывает :-(
		if (window.document.querySelector('.js-submit-error-msg').innerHTML == "Администратор запретил размещать любые ссылки в этой группе") err.add = "Администратор запретил размещать ссылки";   
		iim(wait(6));
	...
	robot.accs().forEach((acc,index)=>{ 
		if((acc==undefined)||(acc.length<5)){} //типа break;
			...
			var line = acc.split('	'), log  = line[0], pass = line[1];
			iimDisplay(`login: ${log} / Pass: ${pass}`); // выводим в окошко аймакроса инфу
			//login_OK(log,pass);							 // входим в учетку ОК
			//iim(wait(15)); 							 // пауза 15 сек
			...
			iim(`CLEAR${wait(20)}`);
			//alert(' change IP'); 
			//NewIP() //  ЕСЛИ НАДО МЕНЯТЬ АЙПИ, НАДО РАЗКОММЕНТИРОВАТЬ (убрать первые 2 дроби!)
			iim(wait(23)); 
		};		
	});
alert('End of Work !');
...
//ФУнкции можно объявлять внизу:

function read(файл) { return imns.FIO.readTextFile(imns.FIO.openNode(файл)).split('\n') }
...
function wait(s) { return `\nWAIT SECONDS=${s}\n`;}
function добавитьВКонец(путь, инфа) { imns.FIO.appendTextFile(imns.FIO.openNode(путь), инфа); iimDisplay(инфа.split('	').join('\n')); }
function quersel(s) { return window.document.querySelectorAll(s) }
...
function NewIP(){var Cc=Components.classes, Ci=Components.interfaces;
