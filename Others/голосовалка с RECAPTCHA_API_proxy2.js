...
var proxy2 = imns.FIO.readTextFile(imns.FIO.openNode("C:\\proxy2.txt")).split('\n');
  
function статус(статус) {var лог = "proxy2.txt:"+статус+"\nСтрока проксей №"+i+"\nип:порт:"+ип+":"+порт+timming(); logi(лог);iimDisplay(лог);}
...
function logi(лог) { imns.FIO.appendTextFile(imns.FIO.openNode("C:\\logi_proxy2.txt"),лог); }
...
for(i=1;i<proxy2.length+1;i++){	
	var ип = proxy2[i].split(':')[0], порт = proxy2[i].split(':')[1]; сменаАйпи(ип,порт);
	 статус("начинаем работу!!")
	var чекПрокси = iimPlayCode("set !timeout_step 30\n URL GOTO=2ip.ru");
	if (чекПрокси == 1)//Page is found!
	{
