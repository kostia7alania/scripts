/* *** DATE: 18.06.2019
 *** аймакрос ваще инвалдиский, не пускает нифига фонового делать...мда. ток синк...
 ***
 *** Исходники сайта особо не меняли уже более года.
 *** */
const rajonSelector = '#step_1>FIELDSET>DIV:nth-of-type(3)>DIV:nth-of-type(2) li'

function letsGo (callback, rajonSelector) {

	const okr = window.setInterval(e=>{
		alert()
		if(document.querySelectorAll(rajonSelector).length) {
			alert('eeee!!!')
			callback()
			clearInterval(okr)
		}
...
letsGo(()=>console.log('cb!', rajonSelector))
